...
ต้องการแค่หน้าเว็บ? ดับเบิลคลิก `start.cmd` (Windows) / `start.command` (macOS) หรือรัน `./start.sh` (Linux) ในโฟลเดอร์ที่แตกไฟล์แล้ว จะเริ่มเซิร์ฟเวอร์และเปิดหน้าเว็บในเบราว์เซอร์ให้
...
ถ้ามี EPUB ของหนังสือเล่มเดียวกัน cue ที่จับคู่ได้จะใช้ข้อความในหนังสือตามต้นฉบับ (รวมเครื่องหมายวรรคตอนและอัญประกาศ) และเมื่อมีเวลาระดับโทเค็นจะถูกแบ่งใหม่ตามขอบเขตประโยคของหนังสือ ทำทีเดียวจบก็ได้ หรือทำภายหลังกับซับไตเติลที่มีอยู่โดยไม่ต้องรัน ASR ใหม่:

```bash
# ถอดเสียงและจัดแนวกับหนังสือในขั้นตอนเดียว
fushi-subs transcribe -l ja --book novel.epub audiobook.m4b -o out.srt

# จัดแนวซับไตเติลที่มีอยู่ภายหลัง; ใช้ out.tokens.jsonl ที่อยู่ข้างกัน (เขียนโดย transcribe -o) เพื่อแบ่งใหม่ตามขอบเขตประโยค
