...
Нужен только веб-интерфейс? Дважды щёлкните `start.cmd` (Windows) / `start.command` (macOS) или запустите `./start.sh` (Linux) в распакованной папке — сервер стартует и страница откроется в браузере.
...
Если есть EPUB той же книги, совпавшие cue берут текст книги как есть (со знаками препинания и кавычками), а при наличии времён по токенам заново разбиваются по границам предложений книги. Либо за один проход, либо позже для готовых субтитров без повторного ASR:

```bash
# Транскрибировать и сразу выровнять по книге
fushi-subs transcribe -l ja --book novel.epub audiobook.m4b -o out.srt

# Позже выровнять готовые субтитры; подхватывает соседний out.tokens.jsonl (его пишет transcribe -o), чтобы заново разбить по границам предложений
