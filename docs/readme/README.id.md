...
Hanya butuh antarmuka web? Klik dua kali `start.cmd` (Windows) / `start.command` (macOS) atau jalankan `./start.sh` (Linux) di folder hasil ekstrak — server dijalankan dan halamannya dibuka di browser.
